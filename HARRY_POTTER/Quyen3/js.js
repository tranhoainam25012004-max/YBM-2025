...
function renderPage(page) {
            const paragraphsToRender = pages[page - 1];
            let html = '';
            
            // Regex nhận diện các ký tự đặc trưng của tiếng Việt
            ...
            paragraphsToRender.forEach(p => {
                ...
                // Tách chuỗi bằng ngoặc đơn để lấy từng phần (Cả ngoài ngoặc và trong ngoặc)
                const tokens = p.split(/(\([^)]+\))/g); 
                
                tokens.forEach(token => {
                    if (token.startsWith('(') && token.endsWith(')')) {
                        let innerText = token.slice(1, -1);
                        
                        // Kiểm tra xem trong ngoặc có phải là tiếng Việt không
                        if (viChars.test(innerText)) {
                            // Đúng là tiếng Việt -> Đóng gói thành cụm song ngữ
                            parsedHtml += `<span class="interactive-group"><span class="en-text">${currentEng.trim()}</span><span class="vi-text">(${innerText.trim()})</span></span> `;
                            currentEng = ''; // Reset tiếng Anh cho cụm tiếp theo
                        } else {
                            // Không có tiếng Việt -> Đây là ngoặc của tiếng Anh, ghép trả lại
                            currentEng += token;
                        }
                    } else {
                        currentEng += token; // Chữ tiếng Anh bình thường
                    }
                });
                
                // Gom nốt phần tiếng Anh nếu bị thừa ở cuối câu
                if (currentEng.trim().length > 0) {
                     parsedHtml += `<span class="en-text">${currentEng.trim()}</span>`;
                }
            ...
            // ... (Các đoạn code disable nút bấm và xóa rác của bạn giữ nguyên)
